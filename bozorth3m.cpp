...
/***********************************************************************/
struct xyt_struct * Bozorth3_Core::bz_load(const MINUTIAE_VECTOR &minutiae, bool useQuality)
{
    int nminutiae;
    int i;
    int nargs_expected;
    ...
    if ( fp == (FILE *) NULL )
    {
        //fprintf( stderr, "ERROR: fopen() of minutiae file \"%s\" failed: %s\n",xyt_file, strerror(errno) );
        return XYT_NULL;
    }*/
    ...
    for (auto minutia : minutiae) {
        xvals_lng[nminutiae] = std::get<0>(minutia).x();
        yvals_lng[nminutiae] = std::get<0>(minutia).y();
        tvals_lng[nminutiae] = std::get<2>(minutia);
        if (useQuality) qvals_lng[nminutiae] = std::get<3>(minutia);
...
void BozorthMultiThreadManager::setParameters(int _numThreads, QMap<QString,MINUTIAE_VECTOR> _minDataAll, FINGERPRINT_PAIRS _fingerprint_pairs)
{
...
BozorthThread::BozorthThread(QMap<QString, MINUTIAE_VECTOR> _minDataAll, FINGERPRINT_PAIRS _thread_fingerprint_pairs):
...
void BozorthThread::loadXYT()
{
    ...
    this->xytData.clear();
    QMapIterator<QString,MINUTIAE_VECTOR> iter(this->minDataAll);
    while (iter.hasNext()) {
        iter.next();
